...
export default function Login({ parkingName, logoUrl, onLoginSuccess }: LoginProps) {
  ...
  const handleLogin = async (event: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.message || 'NÃ£o foi possÃ­vel entrar no sistema.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-5xl overflow-hidden rounded-3xl bg-white shadow-2xl shadow-slate-900/10 grid md:grid-cols-[1.05fr_.95fr]">
        <section className="relative overflow-hidden bg-indigo-700 px-8 py-10 sm:px-12 sm:py-14 text-white flex flex-col justify-between min-h-[350px]">
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div><p className="text-lg font-black tracking-tight">{brand}</p><p className="text-xs text-indigo-100 font-medium">GestÃ£o inteligente de estacionamento</p></div>
          </div>
          <motion.div initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: .45 }} className="relative my-10">
            ...
            <h1 className="mt-5 text-3xl sm:text-4xl font-black leading-tight">Controle do seu estacionamento, em um sÃ³ lugar.</h1>
            <p className="mt-4 max-w-md text-sm leading-relaxed text-indigo-100">Acesse o painel para registrar entradas, acompanhar o pÃ¡tio e movimentar o caixa.</p>
          </motion.div>
          <p className="relative text-xs text-indigo-200">Â© {new Date().getFullYear()} {brand}</p>
        </section>
          ...
          <div className="w-full max-w-sm mx-auto">
            {screen === 'login' ? (
              <form onSubmit={handleLogin} className="space-y-4">
                ...
                <p className="text-sm text-slate-500">Informe seu nome de usuÃ¡rio ou e-mail e a sua senha.</p>
                <Field label="Nome de usuÃ¡rio ou e-mail" icon={<UserRound className="w-4 h-4" />} value={identifier} onChange={(value) => { setIdentifier(value); clearError(); }} placeholder="Seu nome de usuÃ¡rio ou e-mail" autoComplete="username" />
